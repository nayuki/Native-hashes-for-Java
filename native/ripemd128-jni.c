...
#define STATE_LEN 4
extern void ripemd128_compress_block(const jbyte *block, uint32_t state[STATE_LEN]);


/*
 * Class:     nayuki_nativehash_Ripemd128
 * Method:    compress
 * Signature: ([I[BII)Z
 */
JNIEXPORT jboolean JNICALL Java_nayuki_nativehash_Ripemd128_compress(JNIEnv *env, jclass thisClass, jintArray stateArray, jbyteArray msg, jint off, jint len) {
	...
	for (newoff = 0; newoff < newlen; newoff += 64)
		ripemd128_compress_block(block + off + newoff, state);
