...
void whirlpool_compress_block(const jbyte block[restrict static 64], uint8_t state[restrict static 64]) {
	...
	for (i = 0; i < 64; i++)
		state[i] ^= (uint8_t)block[i] ^ (uint8_t)(tempBlock[i >> 3] >> ((i & 7) << 3));
}
