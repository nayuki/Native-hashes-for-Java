...
void ripemd256_compress_block(const jbyte block[static 64], uint32_t state[static 8]) {
	...
	state[4] = 0U + state[4] + al;
	state[5] = 0U + state[5] + bl;
	state[6] = 0U + state[6] + cl;
	state[7] = 0U + state[7] + dl;
}
